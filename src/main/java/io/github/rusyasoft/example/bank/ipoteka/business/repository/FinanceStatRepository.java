...
public interface FinanceStatRepository extends CrudRepository<FinanceStat, Long> {
    ...
    @Query(value = "select year, name, summa from bank inner join " +
            "(select year, bank_id, sum(amount) as summa from finance_stat group by year, bank_id order by year, bank_id) " +
            " as ft on bank.id = ft.bank_id"
            , nativeQuery = true)
